...
public class AuthController {
    ...
    //그리고 기본값도 설정이 가능함
    //그리고 다른 타입도 간으하며 여러개의 RequestParam도 받을 수 있다.
    //int 는 null을 허용안하기 때문에 값이 없음의 상태
    //그리서 required = false를 했지만 에러가 뜸 => Integer로 해야 null로 받을 수 있다.
    ...
    //name은 필수 x , 이메일은 기본값으로 no-email
    ...
    //@RequestBody
    //HTTP 요청의 바디에 들어있는 JSON 데이터를 자바객체(DTO)로 변환해서 주입해주는 어노테이션
    //클라이언트가 JSON 형식으로 데이터를 보냄
    //백엔드 서버는 그 JSON을 @RequestBody가 붙은 DTO로 자동 매핑
    //일반적오로 POST , PUT , PATCH 에서 사용
    ...
    public String signin(@RequestBody SigninReqDto signinReqDto){
        ...
        return signinReqDto.getEmail() + "님 반갑습니다.";
    }
}
